"""
Studio Onyrix v0.6 - AI-Powered DAW Project System
...
def cmd_new_project(args):
    ...
    CURRENT_PROJECT = DAWProject(
        ...
        style_tags=[
            tag.strip() for tag in kwargs.get("style", "").replace(",", " ").split() if tag.strip()
        ],
    )
...
def cmd_arrangement(args):
    ...
    if kwargs.get('description'):
        CURRENT_PROJECT.description = kwargs['description']
    if kwargs.get('style'):
        CURRENT_PROJECT.style_tags = [
            tag.strip() for tag in kwargs['style'].replace(",", " ").split() if tag.strip()
        ]
    if kwargs.get('master'):
        CURRENT_PROJECT.master_volume = float(kwargs['master'])
    save_project()
...
def cmd_compose_song(args):
    ...
    if render_mode in ('musicgen', 'ai'):
        ...
        paths = renderer.render_project(
            CURRENT_PROJECT,
            output_dir,
            duration=int(kwargs['duration']) if kwargs.get('duration') else None,
        )
    else:
    ...
    if paths.get('musicgen_wav'):
        print(f"  MusicGen WAV: {paths['musicgen_wav']}")


def cmd_styles(args):
    """List parametric song styles."""
    print("\nAvailable v0.6 song styles:")
    for style in available_styles():
        print(f"  {style}")


def cmd_project_json(args):
    """Export or print the v0.6 project JSON document."""
    kwargs = parse_flags(args)
    data = CURRENT_PROJECT.to_project_document()

    if kwargs.get('print'):
        print(json.dumps(data, indent=2, default=str))
        return

    filepath = args[0] if args and not args[0].startswith("--") else kwargs.get('file', PROJECT_FILE)
    os.makedirs("output", exist_ok=True)
    if not filepath.startswith("output"):
        filepath = os.path.join("output", filepath)
    with open(filepath, 'w') as f:
        json.dump(data, f, indent=2, default=str)
    print(f"Project JSON v{data['schema_version']} exported: {filepath}")
    print(f"  Tracks: {len(data['tracks'])}")
    print(f"  Parts: {len(data['parts'])}")
    print(f"  Audio assets: {len(data['assets']['audio'])}")
    print(f"  Memory events: {len(data['memory'].get('generation_history', []))}")
...
def cmd_show(args):
    ...
    print(f"Groove: {CURRENT_PROJECT.groove} | Swing: {CURRENT_PROJECT.swing:.0%}")
    print(f"Style: {', '.join(CURRENT_PROJECT.style_tags) if CURRENT_PROJECT.style_tags else '-'}")
    print(f"Schema: {CURRENT_PROJECT.schema_version}")
    print(f"Memory events: {len(CURRENT_PROJECT.memory.get('generation_history', []))}")
...
PROJECT:
  ...
  json --print                   Print v0.6 project JSON
...
ADDING PARTS:
  add [flags]                    Add and generate a part
    Flags:
      ...
      --style "trap dark"        Set project style tags
